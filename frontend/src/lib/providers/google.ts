    ...
    if (!res.ok) {
      ...
      if (res.status === 429 || lower.includes('quota') || lower.includes('rate')) {
        throw new Error(
          ...
          `Try: switch to "Gemini 2.0 Flash" in Settings, wait 60s, or check https://ai.google.dev/gemini-api/docs/rate-limits. ` +
      ...
      if (res.status === 403) {
        throw new Error(`Gemini access denied (403). The key may be on a project without the Generative Language API enabled. Detail: ${detail.slice(0, 200)}`);
      }
